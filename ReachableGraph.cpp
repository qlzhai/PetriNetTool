...
std::vector<std::pair<bool, int>> v_old_;
std::vector<MarkingNode> nodes_;
std::mutex mtx_v_new_;
std::mutex mtx_v_old_;

// 定义条件变量
std::condition_variable cv_v_new_;
std::condition_variable cv_v_old_;

// 定义线程函数
void ReachableGraph::worker() {
	while (true) {
		std::unique_lock<std::mutex> lock(mtx_v_new_);
		cv_v_new_.wait(lock, [this] { return !v_new_.empty(); });

		// 取出一个任务
		Eigen::VectorXi present_marking = v_new_.back();
		v_new_.pop_back();

		lock.unlock();

		int present_index = GetNodeNumberInVOld(present_marking);

		std::vector<int> firable_t = this->FreshFirableTransition(present_marking);
		unsigned int length = firable_t.size();
		for (int i = 0; i < length; i++) {
			int transition_name = firable_t[i];

			Eigen::VectorXi follow_marking = this->FiringATransition(present_marking, transition_name);
			int follow_index = GetNodeNumberInVOld(follow_marking);
			if (follow_index == -1) {
				std::unique_lock<std::mutex> lock(mtx_v_old_);
				marking_number_++;
				v_new_.emplace_back(follow_marking);
				v_old_.emplace(Vector2String(follow_marking), std::make_pair(false, marking_number_));
				AddNode(marking_number_, follow_marking);
				follow_index = marking_number_;
				lock.unlock();
			}
...
bool ReachableGraph::BuildReachabilityGraph2() {
	this->nodes_.clear();
	v_new_.clear();
	v_old_.clear();
	marking_number_ = 0;
	...
	Eigen::VectorXi marking_zero = PetriNet::getInstance().m0_;
	v_new_.emplace_back(marking_zero);
	v_old_.emplace(Vector2String(marking_zero), std::make_pair(false, marking_number_));
	AddNode(marking_number_, marking_zero);

	int num_threads = std::thread::hardware_concurrency(); // 获取可用的CPU核心数
	std::vector<std::thread> threads(num_threads);

	for (int i = 0; i < num_threads; i++) {
		threads[i] = std::thread(&ReachableGraph::worker, this);
	}

	while (!v_new_.empty()) {
		{
			std::lock_guard<std::mutex> lock(mtx_v_new_);
			cv_v_new_.notify_all();
		}

		// 等待当前迭代完成
		{
			std::unique_lock<std::mutex> lock(mtx_v_old_);
			cv_v_old_.wait(lock, [this] { return v_new_.empty(); });
		}
	}
